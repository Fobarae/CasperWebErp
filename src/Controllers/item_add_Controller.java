...
public class item_add_Controller extends AboveGod {
    ...
    public void add_Item(javafx.event.ActionEvent event) throws IOException {
        ...
        if (event.getSource() == AddB) {
            ...
            Item item = new Item(type, price, recurring);
            ...
            ((Label)box.getChildren().get(1)).setText(item.getPrice());

            ((Label)box.getChildren().get(2)).setText(item.getRecurring());

            pnItems.getChildren().add(box);

            invoiceTypes.add(new Invoice(item.getType(),Integer.valueOf(item.getPrice()),item.getRecurring()));
